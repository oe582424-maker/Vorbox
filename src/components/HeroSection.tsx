  ...
  return (
    <section id="hero-section" className="relative bg-neutral-900 text-white overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-20 lg:py-24">
        <div className={`grid grid-cols-1 ${isDropActive ? 'lg:grid-cols-12 gap-8 lg:gap-12' : 'gap-8 max-w-4xl'} items-center`}>
          ...
          <div className={`${isDropActive ? 'lg:col-span-7' : 'w-full'} flex flex-col items-start space-y-5 sm:space-y-6 text-left`}>
            ...
            {/* Value Props Bar */}
            <div className="grid grid-cols-3 gap-2 sm:gap-4 pt-6 border-t border-neutral-800 w-full text-neutral-300">
          ...
          {/* Visual Showcase Card (Featured Drop) */}
          {isDropActive && drop && (
            <div className="lg:col-span-5 relative mt-4 lg:mt-0">
              <div 
                id="hero-featured-drop-card"
                onClick={onFeaturedDropClick}
                className="relative rounded-2xl overflow-hidden bg-neutral-800 border border-neutral-700 shadow-2xl group cursor-pointer transition-all hover:border-neutral-500"
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="text-[11px] uppercase tracking-wider text-emerald-400 font-mono font-semibold">
                        {drop.badgeText || 'Featured Drop'}
                      </span>
                      <h3 className="text-lg font-bold text-white font-mono">{drop.title}</h3>
                      {drop.subtitle && <p className="text-xs text-neutral-300">{drop.subtitle}</p>}
                    </div>
                    <div className="text-right shrink-0 ml-3">
                      <span className="text-lg font-black text-white font-mono">৳{drop.price}</span>
